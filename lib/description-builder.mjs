...
/**
 * Extract key fields for human-readable report preparation.
 * 
 * IMPORTANT: This does NOT submit to any API. Baltimore 311 API does not work.
 * These fields are for human review only - to help users understand what
 * information would be relevant when manually reporting via phone or in person.
 * 
 * @param {object} params - Report parameters
 * @param {string} params.category - AI category
 * @param {string|null} params.subcategory - Baltimore subcategory
 ...
 * @returns {object} Structured fields for human review
 */
export function buildReportFields({
  ...
  return {
    issue_type: subcategory || category,
    description: userDescription || `${taxonomy?.displayName || category} reported via photo`,
    latitude,
    longitude,
    address: locationAddress,
    photo_note: 'Photo available for reference when reporting',
  };
}
