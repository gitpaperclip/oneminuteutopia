...
const INCIDENT_ID = /^[A-Za-z0-9_-]{21}$/;
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json(
      { error: known ? error.message : 'Confirmation data is temporarily unavailable.' },
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const known = error instanceof HttpError;
    return NextResponse.json(
      { error: known ? error.message : 'Could not save that confirmation.' },
      { status: known ? error.status : 503, headers: { 'Cache-Control': 'no-store' } },
    );
  }
}
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const known = error instanceof HttpError;
    return NextResponse.json(
      { error: known ? error.message : 'Could not remove that confirmation.' },
      { status: known ? error.status : 503, headers: { 'Cache-Control': 'no-store' } },
    );
  }
}
